			...
			<form action="{{ route("blog.update", $model->id) }}" id="regform" class="form-horizontal" enctype="multipart/form-data" method="post" accept-charset="utf-8">
				...
				<div class="box box-info">
					<div class="box-body">
                        ...
                        <div class="col-sm-8"><textarea class="form-control ckeditor" id="description" name="description">{{ $model->description }}</textarea>
                        <span style="color: red">{{ $errors->first("description") }}</span></div></div>

                        @if($model->attachment)
							<div class="form-group">
								<label for="image" class="col-sm-2 control-label">Exist Attachment </label>
								<div class="col-sm-8">
                                    @if($model->extension=='jpg' || $model->extension=='png' || $model->extension=='jpeg')
                                        <img id="preview" src="{{ asset('public/admin/images/blogs') }}/{{ $model->attachment }}" width="100px" alt="">
                                    @else
