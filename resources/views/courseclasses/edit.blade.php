...
@section('title', 'Edit CourseClass')
@push('css')
    <style>
        select {
...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<form action="{{ route("courseclass.update", $model->id) }}" id="regform" class="form-horizontal" enctype="multipart/form-data" method="post" accept-charset="utf-8">
				@csrf
                {{ method_field('PATCH') }}
...
<div class="col-sm-8"><input type="number" class="form-control" name="chapter_id" value="{{ $model->chapter_id }}" placeholder="Enter chapter_id">
...
<div class="col-sm-8"><input type="text" class="form-control" name="type" value="{{ $model->type }}" placeholder="Enter type">
...
<div class="col-sm-8"><input type="text" class="form-control" name="attachment" value="{{ $model->attachment }}" placeholder="Enter attachment">
...
<div class="col-sm-8"><select class="form-control" name="status"><option value="1" {{ $model->is_featured==1?"selected":"" }}>Active</option><option value="0" {{ $model->is_featured==0?"selected":"" }}>In Active</option></select><span style="color: red">{{ $errors->first("is_featured") }}</span></div></div><div class="form-group">
