...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<form action="{{ route("enrollstudent.update", $model->id) }}" id="regform" class="form-horizontal" enctype="multipart/form-data" method="post" accept-charset="utf-8">
				@csrf
				{{ method_field('PATCH') }}
				...
				<div class="box box-info">
					<div class="box-body">
							...
							<div class="col-sm-8">
								<select name="user_slug" id="user_slug" class="form-control">
									...
									@foreach ($users as $user)
										<option value="{{ $user->slug }}" {{ $model->user_slug==$user->slug?'selected':'' }}>{{ $user->name }}</option>
									@endforeach
								</select>
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select name="products[]" id="course_slug" class="form-control">
									<option value="" selected>Select Course</option>
									@foreach ($order_courses as $order_course)
										<option value="course,{{ $order_course->order_number }},{{ $order_course->slug }}" {{ $model->product_slug==$order_course->slug?'selected':'' }}>{{ $order_course->title }}</option>
									@endforeach
								</select>
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select name="products[]" id="bundle_slug" class="form-control">
									<option value="" selected>Select Bundle</option>
									@foreach ($order_bundles as $bundle_course)
										<option value="bundle,{{ $bundle_course->order_number }},{{ $bundle_course->slug }}" {{ $model->product_slug==$bundle_course->slug?'selected':'' }}>{{ $bundle_course->title }}</option>
									@endforeach
								</select>
